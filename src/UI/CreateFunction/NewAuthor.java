...
public class NewAuthor extends JFrame {
	 ...
	 */
	public NewAuthor() throws ClassNotFoundException, SQLException {
		...
		int i = 0;
		ResultSet rs = SQLSvConnection.querry("SELECT AuthorID FROM Author");
		while (rs.next()) {
			i = rs.getInt(1);
		}
		int authid = ++i;
			...
			public void mouseClicked(MouseEvent e) {
				if (check(name.getText())) {
					...
					try {
						...
						stm.executeUpdate("INSERT INTO Author VALUES ( " + authid + ", '" + name.getText() + "');");
		...
		JLabel lblAuthid = new JLabel(Integer.toString(authid));
